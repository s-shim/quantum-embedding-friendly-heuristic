...
def LS_Q(bestRevenue,is_sol,indNbr,redConfG,revenueFunction,timeTheory,sampler):
    improve = False
    for (u,q) in redConfG.nodes():
        if is_sol[u,q] == 0 and len(indNbr[u,q]) == 0:
            improve = True
        ...
        if is_sol[u,q] == 1:
            ...
            for (v,r) in redConfG.neighbors((u,q)):
                if len(indNbr[v,r]) == 1:
                    theNeighbors += [(v,r)]
            subConfG = redConfG.subgraph(theNeighbors)
            sub_is_sol, subRevenue, timeTheory, infeasibility = QUBO(subConfG,revenueFunction,is_sol,timeTheory,sampler)
            
            if subRevenue > revenueFunction[u,q] and infeasibility == 0:
                improve = True
                print('### deletion',u,q)
                bestRevenue = bestRevenue - revenueFunction[u,q] + subRevenue 
                print(bestRevenue)
                is_sol = copy.deepcopy(sub_is_sol)
                is_sol[u,q] = 0
                for (v,r) in redConfG.neighbors((u,q)):
...
def QUBO(subConfG,revenueFunction,sub_is_sol,timeTheory,sampler):

    bigM = len(subConfG.nodes()) * 600 + 1
    
    Q1 = {}
    Q2 = {}
    #         
    for (u,q) in subConfG.nodes():
        Q1['X[%s,%s]'%(u,q)] = -float(revenueFunction[u,q])
        
    for ((u,q),(v,r)) in subConfG.edges():
        Q2['X[%s,%s]'%(u,q),'X[%s,%s]'%(v,r)] = bigM
    ...
    tocTheory = time.time()
    timeTheory = timeTheory + tocTheory - ticTheory
    ...
    offered = []
    for (u,q) in subConfG.nodes():
        xVal_u_p = samplesetPandas.loc[[0],'sample'][0]['X[%s,%s]'%(u,q)]
        if xVal_u_p > 1 - 0.0001:
            offered += [(u,q)]
            sub_is_sol[u,q] = 1
            totalRevenue += revenueFunction[u,q]
        if xVal_u_p < 0 + 0.0001:
            sub_is_sol[u,q] = 0
    
    infeasibility = len(confG.subgraph(offered).edges())
    print()
    print('### networkID, rep =',networkID,',',rep)
    print('### Total revenue =',totalRevenue)
    print('### Infeasibility =',infeasibility)    
    
    return sub_is_sol, totalRevenue, timeTheory, infeasibility
...
timeTheory = 0.0
while improve == True:
    bestRevenue,is_sol,indNbr,improve,timeTheory = LS_Q(bestRevenue,is_sol,indNbr,redConfG,revenueFunction,timeTheory,sampler)
