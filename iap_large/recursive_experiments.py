...
def LS(bestRevenue,is_sol,indNbr,redConfG,revenueFunction,timeTheory):
    ...
    for (u,q) in redConfG.nodes():
        if is_sol[u,q] == 0 and len(indNbr[u,q]) == 0:
            improve = True
            #print('# addition',u,q,bestRevenue)
            is_sol[u,q] = 1
        ...
        if is_sol[u,q] == 1:
            ...
            if model.objVal > revenueFunction[u,q]:
                improve = True
                #print('# deletion',u,q,bestRevenue)
...
for (networkID,repNum) in [(0,50),(1,10),(2,50),(3,50),(4,50),(5,50),(6,10),(7,10),(8,50),(9,50)]:
    ...
    for rep in range(repNum):
        ...
        infeasibility = len(redConfG.subgraph(theSolution).edges())
        ...
        netArray += [networkID]
        repArray += [rep]
        methodArray += ['Partition']
        optArray += [finalRevenue]
        timeArray += [elapse]
        timeTheoryArray += [timeTheory]
        infArray += [infeasibility]
        
        partition = pd.DataFrame(list(zip(netArray,repArray,methodArray,optArray,timeArray,timeTheoryArray,infArray)),columns = ['Network','Rep','Method','Opt','Time','theoryTime','Infeasibility'])
        partition.to_csv(r'partition_solver_%s.csv'%(networkID), index = False)#Check
