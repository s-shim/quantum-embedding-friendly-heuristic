...
for i in range(0,10):
    ...
    for rep in range(10):
        ...
        lines = pd.read_csv('../fb/lines/lines_%s.csv'%(networkID))
        lineList = list(zip(lines['Source'], lines['Target']))
        
        # 4. Nodes: Load as normal
        nodes = pd.read_csv('../fb/nodes_zip/nodes_%s_%s.csv'%(networkID,rep))
        ...
        while improve == True:
            bestRevenue,is_sol,indNbr,improve, timeTheory, subproblems = LS(bestRevenue,is_sol,indNbr,redConfG,revenueFunction,timeTheory, subproblems)
        ...
        ticEntire = time.time()
        optRevenue, bestBound, optTime0 = solveGlobalOptimal(redConfG, revenueFunction, seconds_limit=3600 * 24)
        tocEntire = time.time()
        optTime = tocEntire - ticEntire
        ...
        output_file = 'Recursive_Pandas_Log_Improved.xlsx'
        
        # 2. Logic to Append to Excel
        if not os.path.isfile(output_file):
            # If file doesn't exist, create it with a header
            df_new_row.to_excel(output_file, index=False)
        else:
            # If it exists, append without overwriting (requires openpyxl)
            with pd.ExcelWriter(output_file, mode='a', engine='openpyxl', if_sheet_exists='overlay') as writer:
                # Find the last row in the existing sheet to start writing
                try:
                    start_row = writer.book['Sheet1'].max_row
                except:
                    start_row = 0
                df_new_row.to_excel(writer, index=False, header=False, startrow=start_row)
